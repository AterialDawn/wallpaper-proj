...
namespace QuickFont
{
    public class QFont : IDisposable
    {
        ...
        private QFont() { }
        internal QFont(QFontData fontData) { this.fontData = fontData; InitVBOs(); }
        ...
        private void RenderDropShadow(float x, float y, char c, QFontGlyph nonShadowGlyph)
        {
            ...
            if (fontData.dropShadow != null && Options.DropShadowActive)
            {
                ...
                fontData.dropShadow.RenderGlyph(
                    PrintOffset.X + x + (fontData.meanGlyphWidth * Options.DropShadowOffset.X + nonShadowGlyph.rect.Width * 0.5f),
                    PrintOffset.Y + y + (fontData.meanGlyphWidth * Options.DropShadowOffset.Y + nonShadowGlyph.rect.Height * 0.5f + nonShadowGlyph.yOffset), c, true);
            }
        }
        ...
        private void RenderGlyph(float x, float y, char c, bool isDropShadow)
        {
            ...
            if (lastBoundTextureId != sheet.GLTexID)
            {
                GL.BindTexture(TextureTarget.Texture2D, sheet.GLTexID);
                lastBoundTextureId = sheet.GLTexID;
            }
            ...
            Vector2 v1 = new Vector2(x, y + glyph.yOffset);
            Vector2 v2 = new Vector2(x, y + glyph.yOffset + glyph.rect.Height);
            Vector2 v3 = new Vector2(x + glyph.rect.Width, y + glyph.yOffset + glyph.rect.Height);
            Vector2 v4 = new Vector2(x + glyph.rect.Width, y + glyph.yOffset);

            var vbo = VertexBuffers[glyph.page];

            int argb = Options.Colour.ToArgb();

            vbo.AddVertex(PrintOffset + v1, tv1, argb);
            vbo.AddVertex(PrintOffset + v2, tv2, argb);
            vbo.AddVertex(PrintOffset + v3, tv3, argb);

            vbo.AddVertex(PrintOffset + v1, tv1, argb);
            vbo.AddVertex(PrintOffset + v3, tv3, argb);
            vbo.AddVertex(PrintOffset + v4, tv4, argb);
        }
        ...
        public void PrintVBO(string text, Vector2 position, QFontAlignment alignment = QFontAlignment.Left)
        {
        ...
        public void PrintVBO(ProcessedText text, Vector2 position)
        {
            lastBoundTextureId = -1;
            PrintOffset = position;
            PrintOrMeasure(text, false);
        }
        ...
        public void ResetVBOs()
        {
            ...
            {
                foreach (var buffer in fontData.dropShadow.VertexBuffers)
                    buffer.Reset();
            }
        }
        ...
        {
            //The reason that this check is done before the other ones is that we want the dropshadow to be rendered first if there is one.
            if (fontData.dropShadow != null)
            {
                foreach (var buffer in fontData.dropShadow.VertexBuffers)
                    buffer.Draw();
